...
declareRegister(peripheralName = MPU, registerName = MPU_TYPE, addressOffset = 0x0'u32, readAccess = true, writeAccess = false, registerDesc = "MPU Type Register the MPU Type Register indicates how many regions the MPU support")
...
declareField(peripheralName = MPU, registerName = MPU_TYPE, fieldName = SEPARATE, bitOffset = 0, bitWidth = 1, readAccess = true, writeAccess = false, fieldDesc = "Indicates support for separate instruction and data address maps. RAZ. ARMv6-M only supports a unified MPU")
declareRegister(peripheralName = MPU, registerName = MPU_CTRL, addressOffset = 0x4'u32, readAccess = true, writeAccess = true, registerDesc = "MPU Control Register enables the MPU, and when the MPU is enabled, controls whether the default memory map is enabled as a background region for privileged accesses, and whether the MPU is enabled for HardFaults, NMIs")
...
declareRegister(peripheralName = MPU, registerName = MPU_RNR, addressOffset = 0x8'u32, readAccess = true, writeAccess = true, registerDesc = "MPU Region Number Register selects the region currently accessed by MPU_RBAR and MPU_RASR")
...
declareRegister(peripheralName = MPU, registerName = MPU_RBAR, addressOffset = 0xC'u32, readAccess = true, writeAccess = true, registerDesc = "MPU Region Base Address Register holds the base address of the region identified by MPU_RNR")
...
declareRegister(peripheralName = MPU, registerName = MPU_RASR, addressOffset = 0x10'u32, readAccess = true, writeAccess = true, registerDesc = "MPU Region Attribute and Size Register defines the size and access behavior of the region identified by MPU_RNR, and enables that region")
