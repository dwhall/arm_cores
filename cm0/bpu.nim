...
declareRegister(peripheralName = BPU, registerName = BP_CTRL, addressOffset = 0x0'u32, readAccess = true, writeAccess = true, registerDesc = "Breakpoint Control Register provides BPU implementation information, and the global enable for the BPU")
...
declareDimRegister(peripheralName = BPU, registerName = BP_COMP, addressOffset = 0x00000008'u32, dim = 8, dimIncrement = 4, readAccess = true, writeAccess = true, registerDesc = "Breakpoint Comparator Register holds a breakpoint address for comparison with instruction addresses in the Code memory region")
